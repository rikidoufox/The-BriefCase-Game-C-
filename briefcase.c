...
int pick_case_index(int cases[], int cases_size){
    ...
    int selected_case;
    do {
        scanf("%d", &selected_case);
        if (selected_case < 1 || selected_case > 26 || cases[selected_case - 1] == -1) {
            printf("\nPick a case: ");
        }
    } while (selected_case < 1 || selected_case > 26 || cases[selected_case - 1] == -1);
    return selected_case - 1;
};
bool get_decision(void){
    int dec;
    while(1){
